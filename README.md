...
- 마우스리브 기능으로 서브 메뉴를 사라지게 하면 예상 못하게 사라져버려서 별로인것 같다.
  --> 🔨 일관되지 않은 기능은 좋지 않아서 제거 후 다 클릭 닫기로 조정.
- gnb 기능 개선
  --> 🔨 해당 gnb의 서브 메뉴 아닌 다른 서브 메뉴는 닫기 스크립트 추가
- 허가변경 공지사항 스크립트 개선
  --> 🔨 스와이프 플러그인 vertical 버전을 이용하여 깔끔하게 변경.
- 모바일 gnb 배경 개선 - 배경 태그 따로 빼기
- 로고 중앙 정렬 스타일 개선
  --> 🔨 양쪽 너비 합을 뺀 뒤 중앙 정렬. 그 전에는 margin-left auto로 오른쪽 정렬을 해버려서 너비가 이상하게 차지 되었다.
